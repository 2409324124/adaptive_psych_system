...
from scripts.simulate_adaptive_sessions import PERSONAS, simulated_response
    ...
    for config in configs:
        ...
        for persona in PERSONAS:
            ...
            while not session.is_complete:
                ...
                response = simulated_response(session.router, item_index, persona.theta)
...
def print_report(results: dict[str, object]) -> None:
    ...
    for block in results["configs"]:
        ...
        for session in block["personas"]:
            print(
                f"  {session['persona']}: answered={session['answered_count']}, "
                f"mean_se={session['mean_standard_error']:.3f}, "
                f"counts={session['dimension_answer_counts']}"
            )
        print(
